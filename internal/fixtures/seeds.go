...
import (
	"aeroportal-content/domain/news/entity"
	promoEntity "aeroportal-content/domain/promo/entity"
	tagEntity "aeroportal-content/domain/tag/entity"
	"time"
...
func main() {
	...
	// promoRepository := ctn.Resolve("promo_repository").(*postgres.PromoRepository)
	...
	// }
	// err = promoRepository.RemoveAll()
	// if err != nil {
	// 	log.Fatalf("failed to remove all promo: %v", err)
	// }
	// err = tagRepository.RemoveAll()
	// if err != nil {
	...
	promo := getPromoList()
	for _, v := range promo {
		err = promoRepository.Create(v)
		if err != nil {
			log.Fatalf("failed to create promo: %v", v.Name)
		}
	}
}
...
// Получение демо-акций для заполнения
func getPromoList() promoEntity.PromoList {
	promo := promoEntity.PromoList{
		promoEntity.Promo{
			Id:         helpers.GetUuidByString("ebfbe194-a9ff-425f-a36d-e72d3d7e41d0"),
			Name:       "(ci3) Акция 1",
			Author:     "Админов И.И.",
			Active:     true,
			ActiveFrom: time.Now().Unix(),
			...
			Period:     "С сентября по ноябрь",
		},
		promoEntity.Promo{
			Id:         helpers.GetUuidByString("65e3e2fc-18f9-4630-a02b-df50513e9ae9"),
			Name:       "Акция 2",
			Author:     "Иванов И.И.",
			Active:     true,
			ActiveFrom: time.Now().Unix(),
		...
		promoEntity.Promo{
			Id:         helpers.GetUuidByString("0f167859-60b4-43a6-84b4-6dac2bd4d1ee"),
			Name:       "Акция 3",
			Author:     "Круглов И.И.",
			Active:     false,
			ActiveFrom: time.Now().Unix(),
